...
export default function ModuleDetailPage() {
  ...
  async function loadModuleAndProgress() {
    ...
    try {
      ...
      // For MVP, we'll mark all lessons as completed if lessonCompleted is true
      // In a real implementation, you'd track each lesson individually
  ...
  async function completeQuiz(score: number) {
    ...
    try {
      ...
      // If passed, unlock next module (TODO)
      if (passed) {
        // We'll implement module unlocking in the next step
        console.log("Quiz passed! Unlock next module");
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      {/* Quiz Modal (Placeholder - Full quiz coming in next step) */}
      {showQuiz && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-w-lg w-full rounded-lg bg-white p-6">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                onClick={() => {
                  // Temporary: auto-pass for testing
                  completeQuiz(100);
                  setShowQuiz(false);
                }}
